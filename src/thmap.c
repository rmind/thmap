 ...
 * case: it has a fanout of 64 operated using the compare-and-swap (CAS)
 * atomic operation.  The subsequent levels have a fanout of 16 (using
 * 4 bits).  As more levels are created, more blocks of the 32-bit hash
 * value might be generated by incrementing the seed parameter of the
 * hash function.
 *
 * Concurrency
 *
 ...
 *
 * - ROOT LEVEL: The root level is a special case, which is implemented
